...
function build() {
  ...
  const ys = [
    ...new Set(
      rows
        .map((r) =>
          isChg ? String(r["Año"] ?? "") : String(r.FECHA ?? "").slice(0, 4),
        )
        .filter(Boolean),
    ),
  ].sort();
  ...
  filtered = rows.filter(
    (r) =>
      (!y ||
        (activeKey === "chg"
          ? String(r["Año"]) === y
          : String(r.FECHA ?? "").slice(0, 4) === y)) &&
      (!m || activeKey === "chg" || String(r.FECHA ?? "").slice(5, 7) === m) &&
      (!g ||
        c.cols.some((x) =>
          String(r[x] ?? "")
            .toLowerCase()
      ...
      c.cols.every(
        (x) =>
          !cf[x] ||
          String(x === "FECHA" ? displayDate(r[x]) : (r[x] ?? ""))
            .toLowerCase()
            .includes(cf[x]),
      ),
  );
...
function openEdit(id) {
  ...
  $("editFields").innerHTML = c.cols
    .map((x) => {
      ...
      return `<label class="${x === c.notes ? "full" : ""}">${c.labels[x]}${x === c.notes ? `<textarea name="${x}">${esc(r[x])}</textarea>` : `<input name="${x}" type="${x === "FECHA" ? "date" : c.numeric.includes(x) ? "number" : "text"}" step="any" value="${esc(r[x])}">`}</label>`;
    })
    .join("");
...
function saveEdit(e) {
  ...
  c.cols.forEach(
    (x) =>
      (o[x] = c.numeric.includes(x)
        ? Number(fd.get(x) || 0)
        : String(fd.get(x) ?? "").trim()),
  );
  if (activeKey === "chg") {
    ...
    o.FECHA = excelDate(o.FECHA);
    if (!o.FECHA) return toast("La fecha es obligatoria");
  }
  if (editingId == null) {
