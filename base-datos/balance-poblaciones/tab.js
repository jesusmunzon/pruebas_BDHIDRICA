...
let rows=[],filtered=[],page=1,editingId=null,changes=0;
...
function init(){
 $('headRow').innerHTML=COLS.map(c=>`<th>${LABEL[c]}</th>`).join('')+'<th>Acciones</th>';
...
function applyFilters(){
 ...
 filtered=rows.filter(r=>(!y||r.FECHA.slice(0,4)===y)&&(!m||r.FECHA.slice(5,7)===m)&&(!g||COLS.some(c=>String(r[c]??'').toLowerCase().includes(g)))&&COLS.every(c=>!cf[c]||String(c==='FECHA'?displayDate(r[c]):r[c]??'').toLowerCase().includes(cf[c])));
